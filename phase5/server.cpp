...
int main(int argc, char* argv[]) {
    ...
    string server_cert = read_file("certs/server.crt");
    if (server_cert.empty()) {
        cerr << "Error: certs/server.crt not found!\n";
        return 1;
    }
    ...
    cout << "Phase 3 Server listening on port " << port << "...\n";
            ...
            if (FD_ISSET(i, &read_set)) {
                if (i == server_fd) {
                    ...
                    if (client_fd != -1) {
                        ...
                        // Phase 3: Send Server Certificate immediately upon connection
                        string cert_packet = "CERT\n" + server_cert + "\nEOF_CERT\n";
                        send(client_fd, cert_packet.c_str(), cert_packet.length(), 0);
                    }
                } else {
                    char buffer[8192];
                        ...
                        while ((pos = msg.find("\n")) != string::npos) {
                            ...
                            if (!client_keys.count(i)) {
                                if (line.rfind("DH_INIT ", 0) == 0) {
                                    ...
                                    // Phase 3: Proof of Possession (Sign DH public key)
                                    string signature = sign_data(my_pub, "certs/server.key");
                                    string sig_hex = "";
                                    ...
                                    string reply = "DH_ACK " + my_pub + " " + sig_hex + "\n";
                            ...
                            if (!client_names.count(i)) {
                                ...
                                if (dec == "/who") {
                                    ...
                                    if (space_pos != string::npos) {
                                        ...
                                        if (content.rfind("__E2E_MSG__", 0) == 0) {
                                            cout << "[SERVER LOG] " << sender << " -> " << recipient << " (Opaque E2E Ciphertext): " << content.substr(11) << "\n";
                                        } else if (content.rfind("__E2E_INIT__", 0) == 0 || content.rfind("__E2E_ACK__", 0) == 0) {
                                            cout << "[SERVER LOG] " << sender << " -> " << recipient << " (E2E Handshake Key Exchange)\n";
                                        } else {
                                            cout << "[SERVER LOG] " << sender << " -> " << recipient << " (Plaintext): " << content << "\n";
                                        }
