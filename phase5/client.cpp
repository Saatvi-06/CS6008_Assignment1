...
void rotation_thread(string partner, int sock) {
    while (true) {
        sleep(60);
        e2e_mutex.lock();
        if (e2e_sessions.find(partner) == e2e_sessions.end() || !e2e_sessions[partner].active) {
            e2e_mutex.unlock();
            break;
        }
            ...
            if (line.rfind("WHO ", 0) == 0) {
                ...
                if (space_pos != string::npos) {
                    ...
                    if (content.rfind("__E2E_INIT__", 0) == 0) {
                        ...
                        bool is_rotation = !e2e_sessions[sender].current_key.empty();
                        e2e_sessions[sender].previous_key = e2e_sessions[sender].current_key;
                        e2e_sessions[sender].current_key = key;
                        e2e_sessions[sender].active = true;
                        e2e_mutex.unlock();
                        ...
                        if (is_rotation) {
                            cout << "\n[E2E Rotation] " << ts << " | New Key Fingerprint: " << key << "\n> " << flush;
                        } else {
                        ...
                        if (pending_dh.find(sender) != pending_dh.end()) {
                            ...
                            if (is_rotation) {
                                ...
                                // Phase 5 Collision Avoidance: Lexicographical Leader Election
                                // Only the client whose username is alphabetically smaller starts the rotation timer
                                if (my_username < sender) {
                                    thread t(rotation_thread, sender, sock);
                                    t.detach();
                                }
                            }
                        } else {
                        ...
                        e2e_mutex.lock();
                        string curr_key = e2e_sessions.count(sender) ? e2e_sessions[sender].current_key : "";
                        string prev_key = e2e_sessions.count(sender) ? e2e_sessions[sender].previous_key : "";
                        e2e_mutex.unlock();
                        
                        if (curr_key.empty()) {
                            cout << "\n[" << sender << "]: [Encrypted E2E Message but no key established!]\n> " << flush;
                        } else {
                            string dec_inner = decrypt_aes_gcm(enc_inner, curr_key);
                            if (dec_inner.empty() && !prev_key.empty()) {
                                // Try previous key for in-flight messages during rotation!
                                dec_inner = decrypt_aes_gcm(enc_inner, prev_key);
                            }
    ...
    while (true) {
        ...
        if (input.rfind("@", 0) == 0) {
            ...
            if (space_pos != string::npos) {
                ...
                e2e_mutex.lock();
                string key = e2e_sessions.count(partner) ? e2e_sessions[partner].current_key : "";
                e2e_mutex.unlock();
            ...
            if (p.empty()) {
                ...
                e2e_mutex.lock();
                string key = e2e_sessions.count(p) ? e2e_sessions[p].current_key : "";
                e2e_mutex.unlock();
